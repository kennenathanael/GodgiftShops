...
import { setupFirstAdmin } from "@/lib/actions/admin-auth";

const initialState = { error: "" };

export default function SetupForm() {
  const [state, formAction] = useFormState(setupFirstAdmin, initialState);

  return (
    <div className="min-h-screen bg-gray-900 flex items-center justify-center p-6">
      <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-sm">
        <h1 className="text-xl font-bold mb-1 text-center">🎁 GodGiftShop</h1>
        <p className="text-center text-gray-500 text-sm mb-6">Create your first Super Admin account</p>
        ...
        <form action={formAction} className="space-y-4">
          ...
          <input type="password" name="password" placeholder="Password (min 6 chars)" required className="w-full border rounded-lg px-3 py-2" />
